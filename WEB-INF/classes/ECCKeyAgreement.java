...
KeyPair kpW = kpg.genKeyPair();
PrivateKey privKeyW = kpW.getPrivate();
PublicKey pubKeyW = kpW.getPublic();
System.out.println("User W: " + privKeyW.toString());
System.out.println("User W: " + pubKeyW.toString());
...
KeyAgreement ecdhW = KeyAgreement.getInstance("ECDH");
ecdhW.init(privKeyW);
ecdhW.doPhase(pubKeyV,true);
ecdhW.doPhase(pubKeyU,true);
...
System.out.println("Secret computed by W: 0x" + (new BigInteger(1, ecdhW.generateSecret()).toString(16)).toUpperCase());
}
}